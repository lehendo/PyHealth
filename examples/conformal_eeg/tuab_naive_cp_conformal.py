"""Naive (split) Conformal Prediction (BaseConformal) on TUAB Abnormal EEG Detection.
...
Multi-seed usage (recommended for papers):
  python examples/conformal_eeg/tuab_naive_cp_conformal.py \\
      --root downloads/tuab/v3.0.0/edf --n-seeds 5 --seed 42 --alpha 0.1 \\
      --log-file tuab_naive_cp_alpha0.1_5seeds.log
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Naive (split) conformal prediction (BaseConformal) on TUAB abnormal EEG detection."
    )
    parser.add_argument(
        "--root",
        type=str,
