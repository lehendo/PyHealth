"""Naive (split) Conformal Prediction (BaseConformal) on TUEV EEG Events.

Same experiment harness as ``tuev_conventional_conformal.py`` (fixed TUH eval test set,
...
matching the naive split-CP baseline on ``sidruns`` (default score: predicted probability
of the true class; threshold from the calibration quantile).

Single-seed usage (from repo root):
  python examples/conformal_eeg/tuev_naive_cp_conformal.py --root downloads/tuev/v2.0.1/edf
  python examples/conformal_eeg/tuev_naive_cp_conformal.py --model tfm --root downloads/tuev/v2.0.1/edf

Multi-seed usage (recommended for papers):
  python examples/conformal_eeg/tuev_naive_cp_conformal.py \\
      --root downloads/tuev/v2.0.1/edf --model tfm --n-seeds 5 --seed 42 --alpha 0.1 \\
      --log-file tuev_naive_cp_tfm_alpha0.1_5seeds.log
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Naive (split) conformal prediction (BaseConformal) on TUEV EEG events."
    )
    parser.add_argument(
        "--root",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Root folder of fine-tuned TFM classifier checkpoints (used only with --model tfm). "
             "Expected sub-folders: <weights-dir>/<basename>_1/best_model.pth, ..., _5/best_model.pth.",
    )
    parser.add_argument(
        "--tokenizer-weights",
        type=str,
...
def _run_one_seed(
    ...
    results = {}
    for alpha in alphas:
        print(f"  Calibrating BaseConformal predictor (alpha={alpha})...")
        predictor = BaseConformal(model=model, alpha=float(alpha))
        predictor.calibrate(cal_dataset=cal_ds)

        y_true, y_prob, _, extra = Trainer(model=predictor, enable_logging=False).inference(
...
def _print_multi_seed_summary(
    ...
    print("\n" + "=" * 80)
    print(f"Per-run results — alpha={alpha} (BaseConformal, fixed test set = TUH eval partition)")
    print("=" * 80)
    ...
    print("  Method: BaseConformal (naive split CP)")
    print("=" * 80)
